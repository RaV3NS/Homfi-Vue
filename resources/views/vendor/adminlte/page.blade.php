            ...
            <div class="{{config('adminlte.classes_topnav_container', 'container')}}">
                ...
                @else
                    <a href="{{ $dashboard_url }}" class="navbar-brand {{ config('adminlte.classes_brand') }}">
                        ...
                        <img src="{{ asset(config('adminlte.logo_img', 'vendor/adminlte/dist/img/AdminLTELogo.png')) }}" alt="{{config('adminlte.logo_img_alt', 'AdminLTE')}}" class="{{ config('adminlte.logo_img_class', 'brand-image elevation-3') }}" style="opacity: 1">
                    </a>
                @endif
                ...
                <ul class="navbar-nav ml-auto @if(config('adminlte.layout_topnav') || View::getSection('layout_topnav'))order-1 order-md-3 navbar-no-expand @endif">
                    ...
                    @if(Auth::user())
                        @if(config('adminlte.usermenu_enabled'))
                            <li class="nav-item dropdown user-notifies">
                                <a class="nav-link" data-toggle="dropdown" href="#" aria-expanded="false">
                                    <i class="far fa-bell"></i>
                                    <span class="badge badge-warning navbar-badge">{{ \App\AdminNotification::getNewCount() }}</span>
                                </a>
                                <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                                    <span class="dropdown-item dropdown-header">{{ \App\AdminNotification::getNewCount() }} уведомлений</span>
                                    <div class="dropdown-divider"></div>
                                    @foreach(Auth::user()->notifies() as $notify)
                                        <span href="#" class="dropdown-item
                                            @if($notify->type == App\AdminNotification::TYPE_NEW_ADVERT) bg-success
                                            @elseif ($notify->type == App\AdminNotification::TYPE_COMPLAIN) bg-danger
                                            @elseif ($notify->type == App\AdminNotification::TYPE_ADVERT_MODERATE) bg-warning
                                            @endif
                                                ">
                                            {{--<i class="fas fa-envelope mr-2"></i>--}} {!! $notify->title !!}
                                            <span class="float-right text-muted text-xs">{{ $notify->created_at->format('d.m.Y H:s') }}</span>
                                        </span>
                                        <div class="dropdown-divider"></div>
                                    @endforeach

                                    <a href="{{ route('admin.notifications.index') }}" class="dropdown-item dropdown-footer">Перейти к уведомлениям</a>
                                </div>
                            </li>
