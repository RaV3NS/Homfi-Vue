<template>
    <div>
        ...
        <div class="bottom-panel">
            <button class="btn-link" @click="backToEdit">Назад</button>
            <button class="btn button-primary" @click="publishAdvert">Опубликовать</button>
        </div>
        ...
        <modal name="success" width="500" height="auto" :adaptive="true">
            <img src="/icons/close.svg" alt="close" class="close_modal_btn" @click="$modal.hide('success')">
            <div class="modal-success-wrapper">
                <img class="mb-10 mb-md-20" src="/icons/success-icon.svg" alt="" decoding="async">
                <p>Поздравляем, ваше объявление успешно создано, и отправлено на модерацию!</p>
            </div>
        </modal>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getAdvert: function() {
                axios.get(window.backend_url + `api/user/${window.user.id}/adverts/${this.id}`).then((response) => {
                    this.advert = response.data;

                    this.h1 = response.data.full_address.ru;
                    this.city_name = response.data.city.name_ru;
                });
            },
            getPrice() {
                if (this.advert)
                    return this.advert.price_month;
            },
            getDate() {
               if (this.advert) {
            ...
            getImages() {
                ...
                for (let i = 0; i < 6; i++) {
                    if (this.advert.images[i]) {
                        this.main_images.push({ src: this.advert.images[i]['720p'], alt: 'image' });
                    } else
            ...
            backToEdit() {
                window.location = '/advert/edit/' + this.advert.id;
            },
            publishAdvert() {
                let request = { status: 'moderate' };
                axios.put(window.backend_url + `api/user/${window.user.id}/adverts/${this.advert.id}`, request)
                .then((response) => {
                    // Open Auth Modal
                    ...
                    // Redirect To Cabinet
                    setTimeout(function() {
                        window.location = window.home_url_adverts;
                    }, 2000);
                });
            }
        },
