    ...
    export default {
        ...
        mounted() {
            ...
            if (localStorage.getItem('advert')) {
                let id = JSON.parse(localStorage.getItem('advert')).id;
                window.location = "/advert/edit/" + id;
            }
        },
        methods: {
            ...
            gotoStep(step) {
                if (step > this.step && this.navEnabled) {
                    if (step <= this.max_validated_step)
            ...
            getCities(id) {
                axios.get(window.backend_url + 'api/geo/cities?district_id=' + id).then((response) => {
                    let data = response.data;
                    this.cities = [];
                    data.map((el) => {
                        this.cities.push({
            ...
            updatePhotos(next = true) {
                ...
                axios.put(window.backend_url + `api/user/${window.user.id}/adverts/${this.advert_id}/photo`, request)
                    .then((response) => {
            ...
            validateStep5() {
                if (this.$refs.contacts_form.validate()) {
                    ...
                    this.navEnabled = false;
                    this.updateAdvert(request, 'contact');
                }
            },
            setParameters() {
            ...
            createAdvert(request) {
                axios.post(window.backend_url + 'api/user/' + window.user.id + '/adverts', request)
                    .then((response) => {
                        localStorage.setItem('advert', JSON.stringify({ id: response.data.id, step: this.step }));
                        this.advert_id = response.data.id;
                        this.saveMapCoords();
                    });
            },
            updateAdvert(request, step = 'initial') {
                ...
                if (step === 'parameter')
                    axios.post(window.backend_url + `api/user/${window.user.id}/adverts/${this.advert_id}/parameter`, request)
                        .then((response) => {
                            //console.log('advert updated parameters');
                        });

                if (step === 'option')
                    axios.post(window.backend_url + `api/user/${window.user.id}/adverts/${this.advert_id}/option`, request)
                        .then((response) => {
                            //console.log('advert updated options');
                        });

                if (step === 'contact')
                    axios.post(window.backend_url + `api/user/${window.user.id}/adverts/${this.advert_id}/contact`, request)
                        .then((response) => {
                            window.location = '/adverts/preview/' + this.advert_id;
                        })
        ...
        watch: {
            ...
            streetsSearch: function(value) {
                ...
                let url = window.backend_url + 'api/geo/search?city_id=' + this.city + '&type=streets&query=' + encodeURI(value);
                if (value && value.trim() !== '')
            ...
            step: function(newValue, value) {
                ...
                if (newValue > this.max_validated_step)
                    this.max_validated_step = newValue;
                ...
                switch (value) {
                    ...
                    case 3:
                        ...
                        this.updateAdvert(request, 'option');
                        break;
                }
            },
