...
<script>
    export default {
        ...
        computed: {
            model: {
                get() {
                    return this.$store.state[this.store_prop][this.id].checked
                },
                set(value) {
                    this.$emit('checked', { value: !this.model, id: this.id, text: this.text });
                    this.$store.dispatch(this.store_action, {id: this.id, value: value})
                }
            }
        },
        props: ["text", "id", "store_prop", "store_action"]
    }
</script>
